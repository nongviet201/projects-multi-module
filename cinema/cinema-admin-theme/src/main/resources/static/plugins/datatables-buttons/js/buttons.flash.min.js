...
f.each(a,function(b,c){if(f.isPlainObject(c))K(c);else{if(E){var d=c.childNodes[0],g,e=[];for(g=d.attributes.length-1;0<=g;g--){var n=d.attributes[g].nodeName;var m=d.attributes[g].nodeValue;-1!==n.indexOf(":")&&(e.push({name:n,value:m}),d.removeAttribute(n))}g=0;for(n=e.length;g<n;g++)m=c.createdAttribute(e[g].name.replace(":","_dt_b_namespace_token_")),m.value=e[g].value,d.setAttributeNode(m)}c=I.serializeToString(c);E&&(-1===c.indexOf("<?xml")&&(c='<?xml version="1.0" encoding="UTF-8" standalone="yes"?>'+
